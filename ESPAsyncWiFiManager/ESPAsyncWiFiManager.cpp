...
void AsyncWiFiManager::setupConfigPortal() {
  ...
  server->on("/", std::bind(&AsyncWiFiManager::handleWifi, this,std::placeholders::_1,true)).setFilter(ON_AP_FILTER);
  server->on("/wifi", std::bind(&AsyncWiFiManager::handleWifiRescan, this, std::placeholders::_1,true)).setFilter(ON_AP_FILTER);
  ...
  server->on("/fwlink", std::bind(&AsyncWiFiManager::handleWifi, this,std::placeholders::_1,true)).setFilter(ON_AP_FILTER);  //Microsoft captive portal. Maybe not needed. Might be handled by notFound handler.
...
void AsyncWiFiManager::criticalLoop(){
  if (_modeless)
  {
	...
	if (connect) {
	  ...
	  if (connectWifi(_ssid, _pass, _eid) != WL_CONNECTED) {
...
int AsyncWiFiManager::connectWifi(String ssid, String pass, String eid) {
  ...
  if (ssid != "") {
  	if(eid == ""){
  	  ...
  	  esp_wifi_sta_wpa2_ent_set_identity((uint8_t *)eid.c_str(), strlen(eid.c_str()));
      esp_wifi_sta_wpa2_ent_set_username((uint8_t *)eid.c_str(), strlen(eid.c_str()));
...
void AsyncWiFiManager::handleWifi(AsyncWebServerRequest *request,boolean scan) {
  ...
  scannow= -1 ;
   ...
   page.replace("{v}", "Bloom | Hotspot");
   page += FPSTR(HTTP_STYLE);
   page += FPSTR(HTTP_SCRIPT);
   page += FPSTR(HTTP_HEAD_END);
   page += FPSTR(HTTP_BEGIN_HTML);
  ...
  if (_sta_static_ip) {
   ...
   page += FPSTR(HTTP_WIFI_END);  
   page += FPSTR(HTTP_FORM_START);
   page += FPSTR(HTTP_FORM_BUTTON);
   page += FPSTR(HTTP_END_HTML);
   page += FPSTR(HTTP_ACTION_SCRIPT);
   page += FPSTR(HTTP_END);
...
/** Wifi config page handler */
void AsyncWiFiManager::handleWifiRescan(AsyncWebServerRequest *request,boolean scan) {
  shouldscan=true;
  scannow= -1 ;

   String page = FPSTR(WFM_HTTP_HEAD);
   page.replace("{v}", "Bloom | Hotspot");
   page += FPSTR(HTTP_RESCAN_SCRIPT);
   page += FPSTR(HTTP_RESCAN_STYLE);
   page += FPSTR(HTTP_HEAD_END);
   page += FPSTR(HTTP_RESCAN_BODY_BEGIN);
  ...
  if (_sta_static_ip) {
   ...
   page += FPSTR(HTTP_RESCAN_BODY_END);  
   page += FPSTR(HTTP_RESCAN_FORM_START);
   page += FPSTR(HTTP_RESCAN_FORM_BUTTON);
   page += FPSTR(HTTP_RESCAN_END_HTML);
   page += FPSTR(HTTP_RESCAN_ACTION_SCRIPT);
   page += FPSTR(HTTP_END);
 ...
 void AsyncWiFiManager::resultPage(AsyncWebServerRequest *request) {

    String page = FPSTR(WFM_HTTP_HEAD);
    page += FPSTR(HTTP_Connection_STYLE);
    page += FPSTR(HTTP_SCRIPT);
    page += FPSTR(HTTP_HEAD_END);
    if (checker == true){
     String item = FPSTR(HTTP_connected);
     item.replace("{n}", WiFi.localIP().toString());
     page += item;
    }else{
      page += FPSTR(HTTP_notConnected);
      page += FPSTR(HTTP_notConnected_SCRIPT);
    }
    page += FPSTR(HTTP_END);
